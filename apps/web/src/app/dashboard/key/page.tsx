...
import { calculateCredits } from "@/lib/credits-calc";
import { Upload, AudioLines, X, Info, AlertCircle, Coins } from "lucide-react";
...
const MAX_DURATION = 600; // 10min

export default function KeyShiftPage() {
  ...
  const credits = duration ? calculateCredits(duration, "key") : 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 2. Upload album art (if available)
      ...
      <div className="mb-8">
        <h1 className="text-2xl font-bold flex items-center gap-3">
          <div className="h-9 w-9 rounded-lg bg-primary/10 flex items-center justify-center">
            <AudioLines className="h-4.5 w-4.5 text-primary" />
          </div>
          {t("key.title")}
        </h1>
      ...
      {file && duration && (
        <div className="mt-4 rounded-lg border border-border/60 bg-card p-4">
          <div className="flex items-center justify-between">
            <span className="text-sm text-muted-foreground">{t("key.keyShift")}</span>
